 ...
 */
public class Vector implements Derivator<Vector> {
    ...
    @Override
    public Vector calculate(double t1, double t2, Vector p1, Vector p2) {
        double dT = t2 - t1;
        return new Vector(
                (p2.getMagnitude() - p1.getMagnitude()) / dT,
                (p2.getTheta() - p1.getTheta()) / dT
        );
    }
