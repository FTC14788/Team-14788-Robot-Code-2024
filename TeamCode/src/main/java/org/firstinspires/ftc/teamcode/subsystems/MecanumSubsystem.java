...
import static org.firstinspires.ftc.teamcode.constants.DriveConstnats.drivePIDFSwitch;
import static org.firstinspires.ftc.teamcode.constants.DriveConstnats.forwardZeroPowerAcceleration;
import static org.firstinspires.ftc.teamcode.constants.DriveConstnats.headingPIDFSwitch;
import static org.firstinspires.ftc.teamcode.constants.DriveConstnats.drivePIDFFeedForward;
import static org.firstinspires.ftc.teamcode.constants.DriveConstnats.headingPIDFFeedForward;
import static org.firstinspires.ftc.teamcode.constants.DriveConstnats.translationalPIDFFeedForward;
import static org.firstinspires.ftc.teamcode.constants.DriveConstnats.lateralZeroPowerAcceleration;
import static org.firstinspires.ftc.teamcode.constants.DriveConstnats.secondaryDrivePIDFFeedForward;
import static org.firstinspires.ftc.teamcode.constants.DriveConstnats.secondaryHeadingPIDFFeedForward;
import static org.firstinspires.ftc.teamcode.constants.DriveConstnats.secondaryTranslationalPIDFFeedForward;
import static org.firstinspires.ftc.teamcode.constants.DriveConstnats.translationalPIDFSwitch;
import static org.firstinspires.ftc.teamcode.constants.DriveConstnats.useSecondaryDrivePID;
import static org.firstinspires.ftc.teamcode.constants.DriveConstnats.useSecondaryHeadingPID;
import static org.firstinspires.ftc.teamcode.constants.DriveConstnats.useSecondaryTranslationalPID;
...
import org.firstinspires.ftc.teamcode.localization.PoseUpdater;
...
import org.firstinspires.ftc.teamcode.pathGeneration.PathBuilder;
import org.firstinspires.ftc.teamcode.pathGeneration.PathCallback;
...
import org.firstinspires.ftc.teamcode.util.DriveVectorScaler;
import org.firstinspires.ftc.teamcode.util.FilteredPIDFController;
 ...
 */
@Config
public class MecanumSubsystem implements Subsystem {
    private static MecanumSubsystem instance;
    ...
    private final int BEZIER_CURVE_BINARY_STEP_LIMIT = DriveConstnats.BEZIER_CURVE_BINARY_STEP_LIMIT;
    private final int AVERAGED_VELOCITY_SAMPLE_NUMBER = DriveConstnats.AVERAGED_VELOCITY_SAMPLE_NUMBER;
    ...
    private double holdPointTranslationalScaling = DriveConstnats.holdPointTranslationalScaling;
    private double holdPointHeadingScaling = DriveConstnats.holdPointHeadingScaling;
    ...
    private PIDFController secondaryTranslationalPIDF = new PIDFController(DriveConstnats.secondaryTranslationalPIDFCoefficients);
    private PIDFController secondaryTranslationalIntegral = new PIDFController(DriveConstnats.secondaryTranslationalIntegral);
    private PIDFController translationalPIDF = new PIDFController(DriveConstnats.translationalPIDFCoefficients);
    private PIDFController translationalIntegral = new PIDFController(DriveConstnats.translationalIntegral);
    private PIDFController secondaryHeadingPIDF = new PIDFController(DriveConstnats.secondaryHeadingPIDFCoefficients);
    private PIDFController headingPIDF = new PIDFController(DriveConstnats.headingPIDFCoefficients);
    private FilteredPIDFController secondaryDrivePIDF = new FilteredPIDFController(DriveConstnats.secondaryDrivePIDFCoefficients);
    private FilteredPIDFController drivePIDF = new FilteredPIDFController(DriveConstnats.drivePIDFCoefficients);

    private KalmanFilter driveKalmanFilter = new KalmanFilter(DriveConstnats.driveKalmanFilterParameters);
    ...
    public static MecanumSubsystem getInstance() {
        if (instance != null) {
            ...
            System.out.println("Mecanum Subsystem not instantiated");
            return null;
        }
    }
    ...
    public void initialize() {
        driveVectorScaler = new DriveVectorScaler(DriveConstnats.frontLeftVector);
     ...
     */
    public Pose getPose() {
        return poseUpdater.getEstimatedPose();
    }
    ...
    /**
     * This takes a pose at a timestamp relative to System.nanoTime() and applies it thru a kalman
     * filter to adjust the robot's pose. Use this with data from apriltags etc.
     * @param pose The pose
     * @param timestamp The timestamp
     */
    public void addVisionMeasurement(Pose2d pose, double timestamp) { poseUpdater.addVisionMeasurement(pose, timestamp); }
    ...
    public void holdPoint(BezierPoint point, double heading) {
        ...
        closestPose = currentPath.getClosestPoint(poseUpdater.getEstimatedPose(), 1);
    }
    ...
    public void followPath(Path path, boolean holdEnd) {
        ...
        followingPathChain = false;
        currentPath = path;
        closestPose = currentPath.getClosestPoint(poseUpdater.getEstimatedPose(), BEZIER_CURVE_BINARY_STEP_LIMIT);
    }
    ...
    /**
     * This updates all the Follower's PIDFs, which updates the motor powers.
     */
    public void update() {
                ...
                if (holdingPosition) {
                    ...
                    if (currentPath.isAtParametricEnd()) {
                        if (followingPathChain && chainIndex < currentPathChain.size() - 1) {
                            ...
                            if ((System.currentTimeMillis() - reachedParametricPathEndTime > currentPath.getPathEndTimeoutConstraint()) || (poseUpdater.getVelocity().getMagnitude() < currentPath.getPathEndVelocityConstraint() && MathFunctions.distance(poseUpdater.getEstimatedPose(), closestPose) < currentPath.getPathEndTranslationalConstraint() && MathFunctions.getSmallestAngleDifference(poseUpdater.getEstimatedPose().getTheta(), currentPath.getClosestPointHeadingGoal()) < currentPath.getPathEndHeadingConstraint())) {
    ...
    public void setTeleOpMovementVectors(double forwardDrive, double lateralDrive, double heading, boolean robotCentric) {
        ...
        if (robotCentric) {
            teleopDriveVector.rotateVector(getPose().getTheta());
        }
    ...
    public double getDriveVelocityError() {
        ...
        Vector forwardHeadingVector = new Vector(1.0, poseUpdater.getEstimatedPose().getTheta());
        ...
        Vector lateralHeadingVector = new Vector(1.0, poseUpdater.getEstimatedPose().getTheta() - Math.PI / 2);
    ...
    public Vector getHeadingVector() {
        ...
        headingError = MathFunctions.getTurnDirection(poseUpdater.getEstimatedPose().getTheta(), currentPath.getClosestPointHeadingGoal()) * MathFunctions.getSmallestAngleDifference(poseUpdater.getEstimatedPose().getTheta(), currentPath.getClosestPointHeadingGoal());
        if (Math.abs(headingError) < headingPIDFSwitch && useSecondaryHeadingPID) {
            secondaryHeadingPIDF.updateError(headingError);
            headingVector = new Vector(MathFunctions.clamp(secondaryHeadingPIDF.runPIDF() + secondaryHeadingPIDFFeedForward * MathFunctions.getTurnDirection(poseUpdater.getEstimatedPose().getTheta(), currentPath.getClosestPointHeadingGoal()), -1, 1), poseUpdater.getEstimatedPose().getTheta());
            return MathFunctions.copyVector(headingVector);
        }
        headingPIDF.updateError(headingError);
        headingVector = new Vector(MathFunctions.clamp(headingPIDF.runPIDF() + headingPIDFFeedForward * MathFunctions.getTurnDirection(poseUpdater.getEstimatedPose().getTheta(), currentPath.getClosestPointHeadingGoal()), -1, 1), poseUpdater.getEstimatedPose().getTheta());
        return MathFunctions.copyVector(headingVector);
    }
    ...
    public Vector getTranslationalCorrection() {
        ...
        double x = closestPose.getX() - poseUpdater.getEstimatedPose().getX();
        double y = closestPose.getY() - poseUpdater.getEstimatedPose().getY();
        ...
        if (MathFunctions.distance(poseUpdater.getEstimatedPose(), closestPose) < translationalPIDFSwitch && useSecondaryTranslationalPID) {
    ...
    public Vector getCentripetalForceCorrection() {
        ...
        centripetalVector = new Vector(MathFunctions.clamp(DriveConstnats.centripetalScaling * DriveConstnats.mass * Math.pow(MathFunctions.dotProduct(poseUpdater.getVelocity(), MathFunctions.normalizeVector(currentPath.getClosestPointTangentVector())), 2) * curvature, -1, 1), currentPath.getClosestPointTangentVector().getTheta() + Math.PI / 2 * MathFunctions.getSign(currentPath.getClosestPointNormalVector().getTheta()));
    ...
    public void telemetryDebug(MultipleTelemetry telemetry) {
        ...
        telemetry.addData("heading", getPose().getTheta());
