...
public class DashboardPoseTracker {
    ...
    public DashboardPoseTracker(PoseUpdater poseUpdater) {
        ...
        for (int i = 0; i < TRACKING_SIZE; i++) {
            xPositions[i] = poseUpdater.getEstimatedPose().getX();
            yPositions[i] = poseUpdater.getEstimatedPose().getY();
        }
    ...
    public void update() {
        if (System.currentTimeMillis() - lastUpdateTime > UPDATE_TIME) {
            ...
            xPositions[0] = poseUpdater.getEstimatedPose().getX();
            yPositions[0] = poseUpdater.getEstimatedPose().getY();
        }
    }
