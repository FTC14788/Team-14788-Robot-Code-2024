...
public class Drawing {
     ...
     * @param mecanumSubsystem
     */
    public static void drawDebug(MecanumSubsystem mecanumSubsystem) {
        if (mecanumSubsystem.getCurrentPath() != null) {
            drawPath(mecanumSubsystem.getCurrentPath(), "#3F51B5");
            Point closestPoint = mecanumSubsystem.getPointFromPath(mecanumSubsystem.getCurrentPath().getClosestPointTValue());
            drawRobot(new Pose(closestPoint.getX(), closestPoint.getY(), mecanumSubsystem.getCurrentPath().getHeadingGoal(mecanumSubsystem.getCurrentPath().getClosestPointTValue())), "#3F51B5");
        }
        drawPoseHistory(mecanumSubsystem.getDashboardPoseTracker(), "#4CAF50");
        drawRobot(mecanumSubsystem.getPose(), "#4CAF50");
        sendPacket();
    }
